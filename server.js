...
// For a persistent session store (recommended for production), you would need this:
...
// Initialize the main DB pool first.
// This `db` pool could *also* be used for the session store if you enable `connect-pg-simple`.
const db = new Pool({
...
// For a persistent session store, you would initialize it like this:
// const sessionStore = new pgSession({
//     pool: db, // Use your existing pg.Pool instance
//     tableName: 'session' // Default table name, ensure this table exists in your DB
// });
...
app.use(session({
    ...
    // For persistent sessions, uncomment the line below and the `pgSession` setup above:
    // store: sessionStore,
    cookie: {
        httpOnly: true,
        sameSite: 'lax',
    },
    rolling: true // ✅ FIX: This resets the cookie's maxAge on every response
}));


// ✅ FIX: Simplified isAuthenticated middleware
function isAuthenticated(req, res, next) {
    if (
        req.session &&
        req.session.isAuthenticated
        // With rolling: true and maxAge, you no longer need to check or update req.session.createdAt here.
        // The session's expiration is handled by the cookie itself and rolling renewal.
    ) {
        return next();
    }
...
// ✅ FIX: Consolidated app.listen and updated console message
// Removed the duplicate app.listen at the end of the file.
app.listen(PORT, () => { // Removed '0.0.0.0' argument for cleaner console output
...
// Duplicate app.put("/menu/:id") - retaining as is per instructions, but note this is redundant
...
const pool = new Pool({ // This 'pool' variable is distinct from 'db' and used for orders.
    connectionString: process.env.DATABASE_URL,
    ...
    if (username === adminuser) {
        bcrypt.compare(password, adminPasswordHash, (err, result) => {
            ...
            if (result) {
                ...
                // ✅ Removed manual req.session.createdAt = Date.now(); as 'rolling: true' handles session renewal
                res.redirect("/orders");
            } else {
